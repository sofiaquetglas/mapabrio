...
export default function App() {
  ...
  if (pantalla === "onboarding") return (
    <Onboarding userId={userId} onDone={(id, nombre) => {
      setProyectoId(id);
  ...
  if (pantalla === "diagnostico") return (
    <Diagnostico proyectoId={proyectoId} nombreEmpresa={nombreEmpresa}
      onDone={() => setPantalla("entregable")} />
  );

  if (pantalla === "entregable") return (
    <Entregable proyectoId={proyectoId} nombreEmpresa={nombreEmpresa} />
  );
...
function Onboarding({ userId, onDone }: { userId: string, onDone: (id: string, nombre: string) => void }) {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      onDone(data.id, datos.nombre_empresa);
    } catch(e: any) {
    ...
    <div style={{ minHeight:"100vh", background:"#1A1410", display:"flex" }}>
      ...
      <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:48 }}>
        <div style={{ background:"#FAF7F2", borderRadius:16, padding:"48px 40px", width:"100%", maxWidth:520 }}>
          ...
          {pasos[paso]}
          {error && (
            <div style={{ marginTop:16, padding:"10px 14px", borderRadius:8, background:"#FEE2E2", color:"#991B1B", fontSize:13 }}>
              {error}
            </div>
          )}
          <div style={{ display:"flex", gap:12, marginTop:32 }}>
            {paso > 0 && (
              <button onClick={() => setPaso(p => p-1)}
                style={{ padding:"12px 24px", borderRadius:8, border:"1px solid #DDD5C8", background:"transparent", color:"#9E9088", fontSize:14, cursor:"pointer" }}>
                ← Atrás
              </button>
            )}
            <button onClick={() => paso < 2 ? setPaso(p => p+1) : guardar()}
              disabled={paso===0 && !datos.nombre_empresa || guardando}
...
function Diagnostico({ proyectoId, nombreEmpresa, onDone }: { proyectoId:string, nombreEmpresa:string, onDone:()=>void }) {
  ...
  if (generando) return (
    <div style={{ minHeight:"100vh", background:"#1A1410", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:24 }}>
      <div style={{ width:60, height:60, border:"3px solid #3D2B1F", borderTopColor:"#B8935A", borderRadius:"50%",
        animation:"spin 1s linear infinite" }} />
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      <p style={{ fontFamily:"Georgia,serif", fontSize:28, color:"#FAF7F2", fontWeight:300 }}>Generando tu diagnóstico...</p>
      <p style={{ color:"#9E9088", fontSize:14 }}>La IA está analizando tu negocio</p>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#1A1410", display:"flex" }}>
      <div style={{ width:260, background:"#1A1410", padding:"32px 0", display:"flex", flexDirection:"column", borderRight:"1px solid #3D2B1F", flexShrink:0 }}>
        ...
        <div style={{ padding:"20px 24px" }}>
          <button onClick={generarDiagnostico}
            style={{ width:"100%", padding:"13px", background:"#B8935A", border:"none", borderRadius:8,
              color:"#1A1410", fontSize:13, fontWeight:600, cursor:"pointer" }}>
            ✦ Generar diagnóstico
          </button>
        </div>
      </div>
      ...
      <div style={{ flex:1, padding:"48px 64px", overflowY:"auto" }}>
        ...
        <h1 style={{ fontFamily:"Georgia,serif", fontSize:42, fontWeight:300, marginBottom:32, color:"#FAF7F2" }}>
          {modulo.label}
        </h1>
        ...
        {!sinDatos[modulo.id] ? (
              ...
              <div key={p.id}>
                ...
                <textarea value={respuestas[modulo.id]?.[p.id] || ""} onChange={e => setResp(modulo.id, p.id, e.target.value)}
                  ...
                  style={{ width:"100%", padding:"12px 14px", borderRadius:8, border:"1px solid #3D2B1F",
                    background:"#2A2016", color:"#FAF7F2", fontSize:13, resize:"vertical", outline:"none",
          ...
          <div style={{ padding:32, background:"#2A2016", borderRadius:12, border:"1px dashed #3D2B1F", textAlign:"center" }}>
            ...
            <p style={{ color:"#9E9088", fontSize:13, lineHeight:1.6 }}>
              El diagnóstico marcará esta área como "No desarrollada" e incluirá recomendaciones para construirla.
            </p>
          </div>
        )}
        ...
        <div style={{ display:"flex", gap:12, marginTop:40, paddingTop:32, borderTop:"1px solid #3D2B1F" }}>
          {moduloActivo > 0 && (
            <button onClick={() => setModuloActivo(m => m-1)}
              style={{ padding:"12px 24px", background:"transparent", border:"1px solid #3D2B1F", borderRadius:8, color:"#9E9088", fontSize:14, cursor:"pointer" }}>
              ← Anterior
            </button>
          )}
...
function Entregable({ proyectoId, nombreEmpresa }: { proyectoId:string, nombreEmpresa:string }) {
  return (
    <div style={{ minHeight:"100vh", background:"#1A1410", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:24 }}>
      <div style={{ fontFamily:"Georgia,serif", fontSize:48, color:"#D4AE7A", fontWeight:300 }}>✦</div>
      <h1 style={{ fontFamily:"Georgia,serif", fontSize:36, color:"#FAF7F2", fontWeight:300 }}>
        Diagnóstico de <em style={{ color:"#D4AE7A" }}>{nombreEmpresa}</em>
      </h1>
      <p style={{ color:"#9E9088", fontSize:15, textAlign:"center", maxWidth:400, lineHeight:1.6 }}>
        El entregable completo se generará aquí con la API de Claude.
      </p>
      <div style={{ display:"flex", gap:16, flexWrap:"wrap", justifyContent:"center" }}>
        {["Score de negocio","Posicionamiento","Roadmap 90 días","Plan de ingresos"].map(s => (
          <div key={s} style={{ padding:"12px 20px", border:"1px solid #3D2B1F", borderRadius:8, color:"#9E9088", fontSize:13 }}>{s}</div>
        ))}
      </div>
    </div>
  );
